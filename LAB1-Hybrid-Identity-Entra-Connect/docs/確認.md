...
flowchart LR
    ...
    subgraph OnPrem["オンプレミス環境"]
        addc01["addc01<br>AD DS / DNS"]
        mec01["mec01<br>Entra Connect<br>(Active)"]
        mec02["mec02<br>Entra Connect<br>(Staging)"]
        app01["app01<br>検証端末"]
    end

    EntraID["Microsoft Entra ID"]

    addc01 -->|LDAP / AD Connector| mec01
    addc01 -->|LDAP / AD Connector| mec02

    mec01 -->|Export| EntraID
    mec02 -.->|Import / Sync Only<br>(No Export)| EntraID
...
`corp.local` は non-routable ドメインであり、そのままでは Entra ID のサインインIDとして利用できない。  
そのため、ルーティング可能な UPN サフィックスを追加し、ユーザーへ割り当てる設計とする。

| 項目 | 設定値 |
|------|--------|
| 既定UPNサフィックス | corp.local |
| 追加UPNサフィックス | corp.example.com |
| 使用UPNサフィックス | corp.example.com |
